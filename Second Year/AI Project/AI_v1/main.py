...
def plot_detection_fields(detection_map: np.array, bicubic: bool=True) -> None:
    ...
    # to visualize a 2D numPy array (detection_map) as an image, 
    # where each cell represents the probab of being det at that cell.
    plt.figure(figsize=(8, 8))          # figure window 8*8
...
def parse_args() -> dict:
    ...
    with open(json_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
        for entry in data:
            key = list(entry.keys())[0] # entry.keys=entry(['scenario_2']) -> list(entry.keys)=['scenario_2'] -> list(entry.keys)[0]='scenario_2'
...
def main() -> None:
    ...
    execution_parameters = parse_args() # for exmaple command can be: python main.py scenario_2 0.5
    ...
    detection_map = M.compute_detection_map() # TASK 1 !!!!!!!!!!!!!!!!!!!!!!! 
    ...
    G = build_graph(detection_map=detection_map, tolerance=execution_parameters['tolerance']) # TASK 2 !!!!!!!!!!!!!!!!!!!!!!!
    ...
    solution_plan, nodes_expanded = path_finding(G=G,
                                 ...
                                 locations=POIs, 
                                 initial_location_index=0, # PREGUNTA: que es esto?
